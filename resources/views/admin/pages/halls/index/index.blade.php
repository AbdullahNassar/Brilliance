...
@section('content')
    ...
    <div class="br-pageheader pd-y-15 pd-l-20">
        <nav class="breadcrumb pd-0 mg-0 tx-12">
            ...
            <span class="breadcrumb-item active">Halls Table</span>
        </nav>
        </div><!-- br-pageheader -->
        <div class="br-pagebody">
            <div class="br-section-wrapper">
                <a class="btn btn-oblong btn-outline-primary mg-b-10 float-right" href="{{route('halls.add')}}">Add New</a>
                <h6 class="tx-gray-800 tx-uppercase tx-bold tx-14 mg-b-10">Halls Table</h6>
                <p class="mg-b-25 mg-lg-b-50">Searching, ordering and paging goodness will be immediately added to the table.</p>
                <div class="table-wrapper">
                    <table id="halls_datatable" class="table display responsive nowrap">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Name</th>
                        ...
                        <tbody>
                            @foreach($halls as $item)
                                <tr>
                                    ...
                                    <th>
                                        <a href="{{ route('halls.edit' , ['id' => $item->id]) }}" class="text-primary edit" title="Edit Hall"><i class="fa fa-edit"></i></a>  
                                        <!--<a title="Delete Hall course" href="#" class="text-danger btndelet" id="{{$item->id}}"><i class="fa fa-close"></i></a>-->
                                        @if($item->active == 1)
                                        <form class="unpublish" method="POST" action="{{route('halls.unpublish', ['id' => $item->id])}}">
                                            {{csrf_field()}}
                                        ...
                                        @else
                                        <form class="publish" method="POST" action="{{route('halls.publish', ['id' => $item->id])}}">
                                            {{csrf_field()}}
                                            <button type="submit" class="btn btn-outline-danger btn-icon mg-r-5">
                                        ...
                                        @endif
                                        <a href="{{ route('halls.schedule' , ['id' => $item->id]) }}" class="text-primary edit" title="Hall Schedule"><i class="fa fa-calendar"></i></a>
                                    </th>
                                </tr>
                            @endforeach
                        </tbody>
...
@endsection
@include('admin.pages.halls.index.scripts')