...
@section('content')
    ...
    <div class="br-pageheader pd-y-15 pd-l-20">
        <nav class="breadcrumb pd-0 mg-0 tx-12">
            ...
            <a class="breadcrumb-item" href="{{route('admin.universities')}}">Universities</a>
            <span class="breadcrumb-item active">Universities Table</span>
        </nav>
        </div><!-- br-pageheader -->
        <div class="br-pagebody">
            <div class="br-section-wrapper">
                <a class="btn btn-oblong btn-outline-primary mg-b-10 float-right" href="{{route('universities.add')}}">Add New</a>
                <h6 class="tx-gray-800 tx-uppercase tx-bold tx-14 mg-b-10">Universities Table</h6>
                <p class="mg-b-25 mg-lg-b-50">Searching, ordering and paging goodness will be immediately added to the table.</p>
                <div class="table-wrapper">
                    <table id="universities_datatable" class="table display responsive nowrap">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Name</th>
                        ...
                        <tbody>
                            @foreach($universities as $item)
                                <tr class="@if($item->active == 0) row-unpublished @endif">
                                    ...
                                    <th>
                                        <a href="{{ route('universities.edit' , ['id' => $item->id]) }}" class="text-primary edit" title="Edit university"><i class="fa fa-edit"></i></a>     
                                        <!--<a title="Delete" href="#" class="text-danger btndelet" id="{{$item->id}}"><i class="fa fa-close"></i></a>-->
                                        @if($item->active == 1)
                                        <form class="unpublish" method="POST" action="{{route('universities.unpublish', ['id' => $item->id])}}">
                                            {{csrf_field()}}
                                        ...
                                        @else
                                        <form class="publish" method="POST" action="{{route('universities.publish', ['id' => $item->id])}}">
                                            {{csrf_field()}}
                                            <button type="submit" class="btn btn-outline-danger btn-icon mg-r-5">
...
@endsection
@include('admin.pages.universities.index.scripts')