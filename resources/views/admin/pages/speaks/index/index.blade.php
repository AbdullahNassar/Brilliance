...
@section('content')
    ...
    <div class="br-pageheader pd-y-15 pd-l-20">
        <nav class="breadcrumb pd-0 mg-0 tx-12">
            ...
            <a class="breadcrumb-item" href="{{route('speaks')}}">Speak to Expert</a>
            <span class="breadcrumb-item active">Speak to Expert Table</span>
        </nav>
        </div><!-- br-pageheader -->
        <div class="br-pagebody">
            <div class="br-section-wrapper">
                <h6 class="tx-gray-800 tx-uppercase tx-bold tx-14 mg-b-10">Speak to Expert Table</h6>
                <p class="mg-b-25 mg-lg-b-50">Searching, ordering and paging goodness will be immediately added to the table.</p>
                <div class="table-wrapper">
                    <table id="program_datatable" class="table display responsive nowrap" style="width:100%">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Program</th>
                        ...
                        <tbody>
                            @foreach ($speaks as $item)
                                <tr>
                                    <td>{{$loop->index +1}}</td>
                                    <td>@if($item->program != null) {{$item->program->translate('en', true)->title}}@else Deleted @endif</td>
                                    <td>@if($item->program != null) {{$item->program->university->translate('en', true)->name}}@else Deleted @endif</td>
                                    <td>{{$item->name}}</td>
                                    <td>{{$item->code}} {{$item->phone}}</th>
                                    <td>{{$item->date}}</td>
                                    <td>{{$item->time}}</td>
                                    <td>    
...
@endsection
@include('admin.pages.speaks.index.scripts')