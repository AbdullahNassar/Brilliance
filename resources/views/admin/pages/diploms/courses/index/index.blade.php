...
@section('content')
    ...
    <div class="br-pageheader pd-y-15 pd-l-20">
        <nav class="breadcrumb pd-0 mg-0 tx-12">
            ...
            <a class="breadcrumb-item" href="{{route('admin.diplom.courses')}}">Diploma Courses</a>
            <span class="breadcrumb-item active">Diploma Courses Table</span>
        </nav>
        </div><!-- br-pageheader -->
        <div class="br-pagebody">
            <div class="br-section-wrapper">
                <a class="btn btn-oblong btn-outline-primary mg-b-10 float-right" href="{{route('diplom.courses.add')}}">Add New</a>
                <h6 class="tx-gray-800 tx-uppercase tx-bold tx-14 mg-b-10">Diploma Courses Table</h6>
                        ...
                        <tbody>
                            @foreach($courses as $item)
                                <tr>
                                    ...
                                    <th>
                                        <a href="{{ route('diplom.courses.edit' , ['id' => $item->id]) }}" class="text-primary edit" title="Edit diplom"><i class="fa fa-edit"></i></a>     
                                        <!--<a title="Delete diplom course" href="#" class="text-danger btndelet" id="{{$item->id}}"><i class="fa fa-close"></i></a>-->
                                        @if($item->active == 1)
                                        <form class="unpublish" method="POST" action="{{route('diplom.courses.unpublish', ['id' => $item->id])}}">
                                            {{csrf_field()}}
                                        ...
                                        @else
                                        <form class="publish" method="POST" action="{{route('diplom.courses.publish', ['id' => $item->id])}}">
                                            {{csrf_field()}}
                                            <button type="submit" class="btn btn-outline-danger btn-icon mg-r-5">
...
@endsection
@include('admin.pages.diploms.courses.index.scripts')