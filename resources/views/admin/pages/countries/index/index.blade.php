...
@section('content')
    ...
    <div class="br-pageheader pd-y-15 pd-l-20">
        <nav class="breadcrumb pd-0 mg-0 tx-12">
            ...
            <span class="breadcrumb-item active">Countries Table</span>
        </nav>
        </div><!-- br-pageheader -->
        <div class="br-pagebody">
            <div class="br-section-wrapper">
                <a class="btn btn-oblong btn-outline-primary mg-b-10 float-right" href="{{route('countries.add')}}">Add New</a>
                <h6 class="tx-gray-800 tx-uppercase tx-bold tx-14 mg-b-10">Countries Table</h6>
                <p class="mg-b-25 mg-lg-b-50">Searching, ordering and paging goodness will be immediately added to the table.</p>
                <div class="table-wrapper">
                    <table id="countries_datatable" class="table display responsive nowrap">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Image</th>
                        ...
                        <tbody>
                            @foreach ($countries as $item)
                                <tr>
                                    <th>{{$loop->index +1}}</th>
                                    <th><img src="{{asset('images/countries/'.$item->image)}}" class="wd-150" alt="logo"></th>
                                    <th>{{$item->translate('en', true)->name}}</th>
                                    <th><a target="_blank" title="Edit Country" href="{{ route('countries.edit' , ['id' => $item->id]) }}" class="text-primary edit"><i class="fa fa-edit"></i></a>     
                                        <a title="Delete Country" href="#" class="text-danger btndelet" id="{{$item->id}}"><i class="fa fa-close"></i></a>     
                                    </th>
                                </tr>
                            @endforeach
                        </tbody>
...
@endsection
@include('admin.pages.countries.index.scripts')