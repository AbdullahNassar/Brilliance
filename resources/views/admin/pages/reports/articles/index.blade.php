...
@section('content')
    ...
    <div class="br-pageheader pd-y-15 pd-l-20">
        <nav class="breadcrumb pd-0 mg-0 tx-12">
            ...
            <a class="breadcrumb-item" href="{{route('article.reports')}}">Article Reports</a>
            <span class="breadcrumb-item active">Article Reports Table</span>
        </nav>
        </div><!-- br-pageheader -->
        <div class="br-pagebody">
            <div class="br-section-wrapper">
                <h6 class="tx-gray-800 tx-uppercase tx-bold tx-14 mg-b-10">Article Reports Table</h6>
                <p class="mg-b-25 mg-lg-b-50">Searching, ordering and paging goodness will be immediately added to the table.</p>
                <div class="table-wrapper">
                    <table id="article_datatable" class="table display responsive nowrap">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Article</th>
                        ...
                        <tbody>
                            @foreach ($reports as $item)
                                <tr>
                                    <th>{{$loop->index +1}}</th>
                                    <th>@if($item->article != null) <a target="_blank" href="{{route('article',  ['id'=>$item->article->id ,'slug' => $item->article->translate(app()->getLocale(), true)->slug])}}"> {{$item->article->translate('en', true)->title}} </a> @else Deleted @endif</th>
...
@endsection
@include('admin.pages.reports.articles.scripts')