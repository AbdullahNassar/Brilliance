...
@section('content')
    ...
    <div class="br-pageheader pd-y-15 pd-l-20">
        <nav class="breadcrumb pd-0 mg-0 tx-12">
            ...
            <a class="breadcrumb-item" href="{{route('admin.trainingcategories')}}">Categories</a>
            <span class="breadcrumb-item active">Categories Table</span>
        </nav>
        </div><!-- br-pageheader -->
        <div class="br-pagebody">
            <div class="br-section-wrapper">
                <a class="btn btn-oblong btn-outline-primary mg-b-10 float-right" href="{{route('trainingcategories.add')}}">Add New</a>
                        ...
                        <tbody>
                            @foreach($categories as $item)
                                <tr>
                                    ...
                                    <th>
                                        <a href="{{ route('trainingcategories.edit' , ['id' => $item->id]) }}" class="text-primary edit" title="Edit Diplom"><i class="fa fa-edit"></i></a>     
                                        <!--<a title="Delete diplom" href="#" class="text-danger btndelet" id="{{$item->id}}"><i class="fa fa-close"></i></a>-->
                                        @if($item->status == 1)
                                        <form class="unpublish" method="POST" action="{{route('trainingcategories.unpublish', ['id' => $item->id])}}">
                                            {{csrf_field()}}
                                        ...
                                        @else
                                        <form class="publish" method="POST" action="{{route('trainingcategories.publish', ['id' => $item->id])}}">
                                            {{csrf_field()}}
                                            <button type="submit" class="btn btn-outline-danger btn-icon mg-r-5">
...
@endsection
@include('admin.pages.training.categories.index.scripts')