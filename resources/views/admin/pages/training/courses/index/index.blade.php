...
@section('content')
    ...
    <div class="br-pageheader pd-y-15 pd-l-20">
        <nav class="breadcrumb pd-0 mg-0 tx-12">
            ...
            <a class="breadcrumb-item" href="{{route('admin.trainingcourses')}}">Courses</a>
            <span class="breadcrumb-item active">Courses Table</span>
        </nav>
        </div><!-- br-pageheader -->
        <div class="br-pagebody">
            <div class="br-section-wrapper">
                <a class="btn btn-oblong btn-outline-primary mg-b-10 float-right" href="{{route('trainingcourses.add')}}">Add New</a>
                <h6 class="tx-gray-800 tx-uppercase tx-bold tx-14 mg-b-10">Courses Table</h6>
                <p class="mg-b-25 mg-lg-b-50">Searching, ordering and paging goodness will be immediately added to the table.</p>
                <div class="table-wrapper">
                    <table id="courses_datatable" class="table display responsive nowrap">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Name</th>
                        ...
                        <tbody>
                            @foreach($courses as $item)
                                <tr>
                                    <th class="@if($item->status == 0) row-unpublished @endif">{{$loop->index +1}}</th>
                                    <th class="@if($item->status == 0) column-unpublished @endif">{{$item->name}}</th>
                                    <th class="@if($item->status == 0) row-unpublished @endif">{{$item->category->name}}</th>
                                    <th class="@if($item->status == 0) row-unpublished @endif">{{$item->corporate->name}}</th>
                                    <th class="@if($item->status == 0) row-unpublished @endif">@if($item->status == 0)
                                        Unpublished
                                    ...
                                    <th>
                                        <a href="{{ route('trainingcourses.edit' , ['id' => $item->id]) }}" class="text-primary edit" title="Edit Diplom"><i class="fa fa-edit"></i></a>     
                                        <!--<a title="Delete diplom" href="#" class="text-danger btndelet" id="{{$item->id}}"><i class="fa fa-close"></i></a>-->
                                        @if($item->status == 1)
                                        <form class="unpublish" method="POST" action="{{route('trainingcourses.unpublish', ['id' => $item->id])}}">
                                            {{csrf_field()}}
                                        ...
                                        @else
                                        <form class="publish" method="POST" action="{{route('trainingcourses.publish', ['id' => $item->id])}}">
                                            {{csrf_field()}}
                                            <button type="submit" class="btn btn-outline-danger btn-icon mg-r-5">
...
@endsection
@include('admin.pages.training.courses.index.scripts')