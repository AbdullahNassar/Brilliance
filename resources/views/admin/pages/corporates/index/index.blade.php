...
@section('content')
    ...
    <div class="br-pageheader pd-y-15 pd-l-20">
        <nav class="breadcrumb pd-0 mg-0 tx-12">
            ...
            <span class="breadcrumb-item active">Corporates Table</span>
        </nav>
        </div><!-- br-pageheader -->
        <div class="br-pagebody">
            <div class="br-section-wrapper">
                <a class="btn btn-oblong btn-outline-primary mg-b-10 float-right" href="{{route('corporates.add')}}">Add New</a>
                <h6 class="tx-gray-800 tx-uppercase tx-bold tx-14 mg-b-10">Corporates Table</h6>
                <p class="mg-b-25 mg-lg-b-50">Searching, ordering and paging goodness will be immediately added to the table.</p>
                <div class="table-wrapper">
                    <table id="corporates_datatable" class="table display responsive nowrap">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Corporate</th>
                        ...
                        <tbody>
                            @foreach($corporates as $item)
                                <tr>
                                    <th>{{$loop->index +1}}</th>
                                    <th><a href="{{ route('corporates.profile' , ['id' => $item->id]) }}">{{$item->name}} </a></th>
                                    @foreach($item->contacts as $contact)
                                        ...
                                        @endif
                                    @endforeach
                                    <th>{{$item->status}} | {{$item->next_call}}</th>
                                    <th>
                                        <a href="{{ route('corporates.edit' , ['id' => $item->id]) }}" class="text-primary edit" title="Edit Corporate"><i class="fa fa-edit"></i></a>     
                                        <a href="{{ route('corporates.activity' , ['id' => $item->id]) }}" class="text-primary edit" title="Add Corporate Activity"><i class="fa fa-clock-o"></i></a>  
                                        @foreach($item->contacts as $contact)
                                        @if($contact->default == 1)   
                                        <a href="mailto:{{$contact->email}}" class="text-primary edit" title="Send Mail"><i class="fa fa-envelope-o"></i></a>   
                                        @endif
                                        @endforeach  
                                        <!--<a title="Delete corporate" href="#" class="text-danger btndelet" id="{{$item->id}}"><i class="fa fa-close"></i></a>-->
                                    </th>
                                </tr>
                            @endforeach
                        </tbody>
