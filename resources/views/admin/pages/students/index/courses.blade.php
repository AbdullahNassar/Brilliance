...
@section('content')
    ...
    <div class="br-pageheader pd-y-15 pd-l-20">
        <nav class="breadcrumb pd-0 mg-0 tx-12">
            ...
            <span class="breadcrumb-item active">Student Courses Form</span>
        </nav>
      </div><!-- br-pageheader -->
      <div class="pd-x-20 pd-sm-x-30 pd-t-20 pd-sm-t-30">
        <h4 class="tx-gray-800 mg-b-5">Student Schedule Form</h4>
        <p class="mg-b-0">Forms are used to collect Courses information with different element types of input, select, checkboxes, radios and more.</p>
      </div>
                        ...
                        <tbody>
                            @foreach($students as $item)
                                <tr>
                                    ...
                                    <th><a href="{{ route('lead.profile' , ['id' => $item->id]) }}">{{$item->name}} {{$item->middle_name}} {{$item->last_name}}</a></th>
                                    <th>{{$item->mobile1}}</th>
                                    <th>@if($item->status != null) {{$item->status}} @else Pending @endif</th>
